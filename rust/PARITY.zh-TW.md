...
- **一支守護 input backlog 的測試，只在它被寫出來的那台機器上會過。**它繞過 `ws::session` 實際套用的 gate 一路寫下去，並斷言 8 MiB 之內就會撞到上界——那量到的是某台主機的 line discipline 在停止接收前會吞掉多少，而不是這個上界本身。它從 `a827e3d`（PTY 由阻塞 writer thread 改為 readiness 驅動 I/O）開始變紅，之後的十一個 commit、五個星期都紅著沒有被發現，包含這個移植版被合併的那一次。沒有人看的紅燈比缺少測試更糟，因為它照樣被算進測試清單裡。上界本身從未失效：照 gate 的做法模擬，Linux 上 backlog 峰值正好停在 4 MiB。第一次改寫把「8 MiB 內撞到上界」換成 `peak <= MAX_QUEUED_INPUT + chunk`，而那次改寫在 macOS 上自己就是空的，這一點是 review 抓到的：把 `input_backlog_is_full()` 硬寫成 `false`，也就是上界整個拿掉，它在那裡照樣會過，因為 canonical mode 的 line discipline 是丟棄多餘資料而不是拒絕寫入，於是根本沒有東西排隊。那正是這次改寫要修的同一個缺陷。現在 child 會先把自己的終端切到 raw mode 再停止讀取，兩個核心因此都會套用 backpressure，gate 在兩邊都真的會觸發；negative control 也在兩邊都會失敗：把上界拿掉之後，測試會報告寫入 64 MiB 而 backlog 始終沒有滿。它現在斷言的是兩件事，不是一條較弱的不變量：gate 真的被撞到，以及停在 gate 的呼叫端不會排超過上界再加一個 chunk。
